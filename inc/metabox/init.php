...
class cmb_Meta_Box {
			...
			switch ( $field['type'] ) {
				...
				case 'multicheck':
					...
					foreach ( $field['options'] as $value => $name ) {
						$meta2 = get_post_meta( $post->ID, $field['id'], 'multicheck' == $field['type'] 
						/* If multicheck this can be multiple values */ );
						if ( $meta2 == '' ) { $meta2 = array(); }
						// Append `[]` to the name to get multiple values
						// Use in_array() to check whether the current option should be checked
						echo '<li><input type="checkbox" name="', $field['id'], '[]" id="', $field['id'], $i, '" value="', $value, '"', in_array( $value, $meta2 ) ? ' checked="checked"' : '', ' /><label for="', $field['id'], $i, '">', $name, '</label></li>';
						$i++;
					}
					echo '</ul>';
					echo '<span class="cmb_metabox_description">', $field['desc'], '</span>';
	...
	function save( $post_id)  {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( $type_comp == true && $field['type'] == 'season' ) {
				$closers = array($new['open_month'], $new['open_day'], $new['close_month'], $new['close_day'] );
				$new = $closers;
			}
